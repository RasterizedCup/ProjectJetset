...
public class Portal : MonoBehaviour
{
    ...
    public float clippingOffsetMultiplier = 3.5f;
    public float screenOffsetScaleMulti = 1f;
    ...
    float ProtectScreenFromClipping(Vector3 viewPoint)
    {
        ...
        float dstToNearClipPlaneCorner = new Vector3(halfWidth, halfHeight, playerCam.nearClipPlane * clippingOffsetMultiplier).magnitude;
        float screenThickness = dstToNearClipPlaneCorner;

        Transform screenT = screen.transform;
        bool camFacingSameDirAsPortal = Vector3.Dot(transform.forward, transform.position - viewPoint) > 0;
        screenT.localScale = new Vector3(screenT.localScale.x, screenT.localScale.y, screenThickness) ;
        screenT.localPosition = Vector3.forward * screenThickness * screenOffsetScaleMulti * ((camFacingSameDirAsPortal) ? 0.5f : -0.5f);
