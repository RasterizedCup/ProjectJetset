...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.678)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.678)) + lerp(albedo.rgb, rimColor, float(0.678)) * rimBrighten, rim);
}
#endif
